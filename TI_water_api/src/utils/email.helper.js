...
class EmailHelper {
  constructor() {
    ...
    if (this.emailProvider === 'none') {
      console.log('[EmailHelper] Email disabled. Quote notifications will not be sent. To enable: set credentials for one of SMTP/SendGrid/Mailgun/Resend (Aquatech_api docs/EMAIL_CONFIG.md). Set EMAIL_PROVIDER=none to hide this message.');
    ...
    if (this.emailProvider !== 'none') {
      console.log('[EmailHelper] Email enabled via', this.emailProvider, '– quote notification emails will be sent when configured.');
    }
  }
